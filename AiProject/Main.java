...
public class Main extends Application {
	...
	SimulatedAnnealingSolver minRemValue = new SimulatedAnnealingSolver();
	...
	public void start(Stage primaryStage) {
		...
		ihm.SimulatedAnnealing.setOnAction(e -> {
			afficherLesLabels();
			long start = System.currentTimeMillis();
			int [][] Sudint= sudko(sudokuInit);
			int[][] sudokuResolu = minRemValue.SimulatedAnnealingSolve(Sudint);
			ihm.resolution(sudokuResolu);
			long time = System.currentTimeMillis() - start;
	...
	public int [][] sudko (int boorde [][]){
		String puzzle ="" ;
		for(int i=0; i<boorde.length ;i++)
		for(int j=0 ; j<boorde.length;j++) {
			if(boorde[i][j]!=0)
			puzzle=puzzle+boorde[i][j];
			else 
				if(boorde [i][j]==0)
					puzzle+=".";
		}
	...
	public void activerButtons(){
		...
		ihm.SimulatedAnnealing.setDisable(false);
	}
	...
	public void desactiverButtons(){
		...
		ihm.SimulatedAnnealing.setDisable(true);
	}
	...
	//  Method that allows us to chose one  to resolve from the files in the "sudokuFiles" directory
